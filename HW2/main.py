...
from algo import QAgent, _QAgent
import numpy as np
import time
import cv2
import gym
    ...
    for i in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            if np.random.rand() < epsilon:
                ...
                action = agent.select_action(str(obs.tolist()))
            ...
            step_count += 1
            # 更新Q表
            agent.learn(str(obs.tolist()), action, reward, str(obs_next.tolist()))
            # 机器人移动到下一个state
            obs = obs_next
            if done:
                envs.reset()
            ...
            if args.save_img:
                pic_name = 'pics/pic' + str(i) + '-' + str(step + 1) + '.png'
                scipy.misc.toimage(info, cmin=0.0, cmax=1).save(pic_name)
