...
def output(freqset, k_item_freq, association_rules, frequency_threshold, k, dataset_path_name, min_sup, min_con):
    ...
    if association_rules:
        if not os.path.exists(os.path.join(OUTPUT_PATH, dataset_path_name + '_association_rules')):
            os.makedirs(os.path.join(OUTPUT_PATH, dataset_path_name + '_association_rules'))
        with open(os.path.join(OUTPUT_PATH, dataset_path_name + '_association_rules/' + str(k) + '-items-rules.txt'), 'w') as outfile:
...
def generate_association_rules(k_freqset, itemset_freq, min_con):
    """
    生成关联规则
    ...
    if len(sys.argv) != 5:
        ...
        for k in range(2, itemset_size + 1):
            ...
            if len(pre_freqset) == 0:
                break
            pre_freqset.sort()
            association_rules = generate_association_rules(pre_freqset, itemset_freq, min_con)
